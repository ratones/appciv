...
var ncp = requireNode('ncp');
var request = require('./proxy');
var semver = requireNode('semver');
var del = requireNode('del');

var platform = global.process.platform;
 ...
 /**
  ...
  updater.prototype.getAppPath = function(){
    var appPath = {
      mac: path.join(global.process.cwd(),'../../..'),
      win: path.dirname(global.process.execPath)
    };
  ...
  updater.prototype.getAppExec = function(){
    ...
    var exec = {
      mac: '',
      win: path.basename(global.process.execPath),
      linux32: path.basename(global.process.execPath),
      linux64: path.basename(global.process.execPath)
    };
    ...
    win: function(filename, cb, manifest, temporaryDirectory){
      var destinationDirectory = getZipDestinationDirectory(filename, temporaryDirectory),
          unzip = function(){
            ...
            exec( '"' + path.resolve(path.dirname(global.process.mainModule.filename), 'tools/unzip.exe') + '" -u -o "' +
...
/**
  ...
  function run(path, args, options){
   ...
   updater.prototype.copyApp = function(from,to,cb){
       ncp(from, to, cb);
  };
